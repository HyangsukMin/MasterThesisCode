...
#---------------------------------
# SKODA
# https://github.com/saif-mahmud/self-attention-HAR/tree/a928840b63241ec1743b6a9419f88ab85f0f11d0/configs
#---------------------------------
...
def get_train_val_test(data):
    ...
    xy = []
    # return data
    for i in range(1, number_of_activity):
        data_for_a_single_activity = data[np.where(data[:, 0] == activity_id[i])]
        xy.append(data_for_a_single_activity)
...
# %%
import pickle
data = get_train_val_test(all_data)
pickle.dump(data, open("Datasets/skoda.pkl","wb"))
# %%
#-------------------------------------------------------
# OPPORTUNITY
#-------------------------------------------------------
...
def generate_data(target_filename, label):
    ...
    data_dir = "OpportunityUCIDataset"
    ...
    cp.dump(obj, open(os.path.join(target_filename), 'wb'), protocol=cp.HIGHEST_PROTOCOL)
...
generate_data("./Datasets/opportunity.pkl", "gestures")
generate_data("./Datasets/opportunity_loco.pkl", "locomotion")
# %%
#---------------------------------------------
# Motion Sense
...
def process_motion_sense_accelerometer_files(accelerometer_data_folder_path):
    ...
    for trial_folder in all_trials_folders:
        ...
        label, order = trial_name.split("_")[0], int(trial_name.split("_")[1])
...
# %%
# datasets = process_motion_sense_accelerometer_files("A_DeviceMotion_data")
for k,v in datasets.items():
    v = sorted(v,key = lambda x: x[0])
    values = np.concatenate([v[i][1][0] for i in range(len(v))], axis = 0)
    labels = np.concatenate([v[i][1][1] for i in range(len(v))], axis = 0)
    pickle.dump((values, labels), open("Datasets/motionsense_{:d}.pkl".format(k), "wb"))
# %%
